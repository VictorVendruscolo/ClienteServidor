...
% --- Nomes fixos em portugues -----------------------------------------------
% O babel ja traduz estes rotulos, mas fixa-los aqui garante o texto correto
% mesmo se o documento for compilado numa instalacao LaTeX sem o pacote de
% portugues - caso em que sairia "Contents", "Figure" e "Table" em ingles.
...
% --- Macros das capturas de tela --------------------------------------------
% Se o arquivo existir em figuras/, ele e inserido com uma moldura fina. Se
% nao existir, aparece um retangulo com o nome esperado, e o documento
% continua compilando.
%
%   \tela{arquivo}{legenda}{largura}          uma captura
%   \telaDupla{arq1}{subleg1}{arq2}{subleg2}{legenda}   duas lado a lado
%
\newcommand{\imgtela}[2]{%
  \IfFileExists{figuras/#1}{%
    \fbox{\includegraphics[width=#2]{figuras/#1}}%
  }{%
    \fbox{%
      \begin{minipage}[c][3cm][c]{#2}
...
\newcommand{\tela}[3]{%
  \begin{figure}[H]
    \centering
    \imgtela{#1}{#3\textwidth}
    \caption{#2}
  \end{figure}%
}

\newcommand{\telaDupla}[5]{%
  \begin{figure}[H]
    \centering
...
{\large PLATAFORMA DE MONITORAMENTO INTELIGENTE DE FILAS\\[4pt] EM TEMPO REAL}
...
(\texttt{pthreads}), entre as três permitidas pelo enunciado: multiplexação,
\texttt{fork()} e threads. O servidor cria uma thread para cada conexão aceita.

O que decidiu a escolha foi a fila compartilhada. Com threads, todas enxergam a
mesma memória, e basta um \textit{mutex} para proteger o acesso; o
\textit{broadcast} também alcança os sockets das outras conexões sem nenhum
mecanismo extra. Com \texttt{fork()}, as duas coisas exigiriam comunicação entre
processos, porque cada processo tem memória isolada. A multiplexação com
\texttt{select()} resolveria o compartilhamento igualmente bem, e sem precisar de
\textit{mutex}; ficou de fora por ser a técnica menos trabalhada ao longo do
curso, e o prazo não permitiria aprendê-la a ponto de explicá-la com segurança.

A thread principal do servidor não atende nenhum cliente. Ela apenas aceita
conexões, cria a thread correspondente e volta a aceitar. Isso atende ao
enunciado, que reserva ao servidor principal apenas aceitar conexões, distribuir
eventos e monitorar sockets.

\begin{lstlisting}[caption={Laço principal do servidor, em \texttt{servidor.c}. A thread criada recebe uma \texttt{struct} própria, com o socket, o IP e a porta de origem e o número da conexão.}]
for (;;) {
    ...
    pthread_create(&identificador_thread, &atributos, atende_cliente, dados);
}
\end{lstlisting}
...
static int             quantidade;               /* posicoes em uso */
static pthread_mutex_t fila_mutex;               /* protege os dois */
\end{lstlisting}

O vetor foi preferido à lista ligada pela simplicidade. O limite de 20.000
registros cobre com folga o teste de carga de 10.000 clientes. Esse limite é uma
decisão de implementação declarada
...
corresponde necessariamente a uma de \texttt{recv()}. Quem lê consome um byte por
vez até encontrar a quebra de linha, e devolve à camada de cima exatamente uma
mensagem por chamada.

Respostas de várias linhas usam um cabeçalho e um rodapé fixos. O cliente lê
linha a linha até bater no rodapé, sem precisar saber quantas linhas virão.
...
\subsection{Duas construções que merecem explicação}

O código usa dois recursos que não aparecem nos exemplos vistos em aula. Como são
os pontos que menos se reconhecem numa primeira leitura, ficam explicados aqui.

\vspace{6pt}
\noindent\textbf{A espera com prazo, no cliente.} A thread do menu envia um
comando e precisa aguardar a resposta --- mas quem recebe a resposta é a outra
thread, a que lê o socket. As duas se comunicam por uma variável de condição
(\texttt{pthread\_cond\_timedwait}): a thread do menu dorme até ser avisada, ou
até o prazo de 3 segundos terminar. O prazo é informado como um instante
absoluto, \texttt{time(NULL)} mais 3, porque é isso que a função espera receber.
A espera fica dentro de um \texttt{while}, e não de um \texttt{if}, porque uma
variável de condição pode acordar sem ninguém ter sinalizado; a condição é
reconferida a cada acorde.

\vspace{6pt}
\noindent\textbf{A separação do nome, no servidor.} O comando de cadastro chega
como uma linha só, e o nome pode conter espaços --- então não dá para separar a
linha em palavras. A leitura é feita com
\texttt{sscanf(linha, "\%*s \%d \%d \%n", ...)}. O \texttt{\%*s} lê a primeira
palavra e a descarta, porque o comando já é conhecido. Os dois \texttt{\%d} leem
o número de sequência e o identificador. O \texttt{\%n} não lê nada: ele grava
quantos caracteres foram consumidos até ali, o que marca exatamente onde o nome
começa. O resto da linha, a partir desse ponto, é o nome inteiro.
...
\begin{enumerate}
  ...
  \item \textbf{Serialização de escrita.} Sem ela, uma mensagem de
        \textit{broadcast} disparada por outra thread poderia entrar no meio de
        uma resposta de várias linhas e quebrar o protocolo. Existe um único
        \textit{mutex} de envio, comum a todos os sockets, mantido travado do
        cabeçalho ao rodapé do bloco. Os três \textit{mutex} do servidor são
        sempre obtidos na mesma ordem --- tabela de sessões, envio, fila ---, e
        nenhum caminho do código inverte essa ordem. É isso que descarta a
        possibilidade de impasse (\textit{deadlock}).
...
Não há verificação. Dois usuários podem ter o mesmo identificador \\
\addlinespace
Capacidade da fila &
...
A introdução do enunciado cita algumas funcionalidades. Algumas ficaram fora de
...
Três comportamentos foram identificados durante os testes e ficam declarados
aqui, em vez de contornados.

\vspace{6pt}
\noindent\textbf{Estouro de inteiro no identificador.} O identificador é lido com
\texttt{sscanf(entrada, "\%d", \&id)}. Um número acima do limite do tipo
\texttt{int} sofre estouro silencioso e é gravado com outro valor: em teste,
\texttt{99999999999999} entrou na fila como \texttt{276447231}. Negativos e zero
são recusados, mas o estouro produz um valor positivo, então passa pela
validação. Corrigir exigiria voltar a \texttt{strtol} com verificação de
\texttt{errno}. É um comportamento comum em C e sem efeito sobre o funcionamento
do sistema.

\vspace{6pt}
\noindent\textbf{O teto de clientes é menor que o da fila.} A fila cabe 20.000
usuários, mas a tabela de sessões cabe 12.000
(\texttt{MAX\_SESSOES}). Portanto o número máximo de clientes conectados ao mesmo
tempo é 12.000, e não 20.000. Os dois limites são independentes: um cliente pode
cadastrar vários usuários. O valor de 12.000 cobre com folga o maior teste
exigido, que é de 10.000 clientes.

\vspace{6pt}
\noindent\textbf{O retrato da fila é reescrito por inteiro.} A cada inserção, o
arquivo \texttt{fila.txt} é regravado do começo, e não acrescido de uma linha. O
custo cresce com o tamanho da fila. A escolha foi essa porque o arquivo precisa
refletir o estado atual, no mesmo formato da resposta de \texttt{LIST}; o
registro que só cresce é o \texttt{historico.txt}, que recebe uma linha por
inserção. Com o volume deste trabalho a diferença não aparece.
...
\begin{enumerate}
  \item \textbf{Teste funcional} --- dois clientes reproduzindo a sequência das
        telas do enunciado.
  \item \textbf{Casos especiais} --- um programa conversando direto com
...
2 & A cadastra \texttt{12/Marcos Castro} e \texttt{13/Juarez Soares Pedro} & \texttt{Usuário Marcos Castro adicionado.} & OK \\
3 & B recebe as notificações & \texttt{[Broadcast] Novo usuário:} com os dois nomes & OK \\
4 & B cadastra \texttt{21/Jonas Baleia} e \texttt{11/Pedro Casagrande} & Confirmação em B, \textit{broadcast} em A & OK \\
5 & A escolhe \texttt{2} & Os quatro usuários, entre cabeçalho e rodapé & OK \\
6 & A escolhe \texttt{3} & Só \texttt{21 - Jonas Baleia} e \texttt{11 - Pedro Casagrande}, cadastrados por B & OK \\
7 & A escolhe \texttt{3} de novo & \texttt{Heartbeat: ALIVE}, pois nada mudou desde a verificação anterior & OK \\
8 & Ambos escolhem \texttt{0} & \texttt{Cliente desconectado.} na tela do servidor & OK \\
\bottomrule
\end{tabular}
\end{table}

O passo 6 é onde seguimos o texto da Observação em vez do \textit{print} do
enunciado. O \textit{print} mostra a fila inteira, e o sistema devolve apenas os
usuários cadastrados por outros clientes. O passo 7 mostra o outro lado da mesma
regra: repetida sem novidade, a operação devolve \texttt{ALIVE}, exatamente como
no \textit{print} do enunciado.

Esta execução é a mesma que aparece nas capturas da Seção~7. Os quatro cadastros
ficaram registrados em \texttt{dados/historico.txt}, com data e hora, e podem ser
conferidos na Figura~\ref{fig:persistencia}.
...
\caption{Casos especiais testados e resposta obtida.}\label{tab:casos}\\
\toprule
...
\vspace{6pt}
\noindent\textbf{Três testes dirigidos à delimitação e à serialização.} As duas
peças mais delicadas do protocolo são a separação das mensagens e a garantia de
que um bloco de várias linhas não é partido. Elas foram testadas em separado, por
um programa que fala direto com o socket. Enviadas duas mensagens completas em um
único \texttt{write}, o servidor processou as duas, na ordem certa. Enviada uma
mensagem partida em dois \texttt{write}, com pausa entre eles, o servidor
remontou a linha corretamente. E com 25 listagens longas pedidas enquanto outro
cliente cadastrava sem parar, nenhum bloco saiu partido: todos os
\textit{broadcast} que chegaram durante as listagens apareceram fora dos blocos.
...
As capturas abaixo mostram a tela do servidor no começo e no fim de cada teste.
O par comprova a numeração contínua: a primeira conexão sai como \texttt{[\#1]}
e a última como \texttt{[\#N]}, com a origem de cada uma. É assim que se
identificam todas as conexões sem precisar de uma captura por linha.

\telaDupla%
  {carga-inicio-100.png}{Início: de \texttt{[\#1]} em diante}
  {carga-fim-100.png}{Fim: até \texttt{[\#100]}}
  {Teste com 100 clientes, visto do servidor.}

\telaDupla%
  {carga-inicio-1000.png}{Início: de \texttt{[\#1]} em diante}
  {carga-fim-1000.png}{Fim: até \texttt{[\#1000]}}
  {Teste com 1.000 clientes, visto do servidor.}

\telaDupla%
  {carga-fim-10000.png}{Servidor: até \texttt{[\#10000]}}
  {carga-cliente-fim-10000.png}{Gerador: 10.000 de 10.000, 0 falhas}
  {Teste com 10.000 clientes. À esquerda, a última conexão registrada pelo servidor; à direita, o fecho do gerador, com as 10.000 conexões abertas ao mesmo tempo e nenhuma falha.}

\tela{carga-cliente-1000.png}{Tela do gerador \texttt{./carga 1000}. Ele informa o progresso a cada 100 conexões, o total aberto e o número de falhas, e mantém tudo ativo até o operador pressionar ENTER.}{0.72}
...
Esta seção mostra o sistema em operação, na mesma sequência do teste funcional
descrito na Seção~5.2. Todas as capturas vêm da execução registrada em
\texttt{dados/historico.txt}, reproduzida na Figura~\ref{fig:persistencia}.

\tela{01-servidor-iniciado.png}{Servidor em execução. A primeira linha é \texttt{Servidor iniciado na porta 8080}; em seguida aparecem os dois clientes conectando e desconectando, cada um com seu número de conexão e sua origem.}{0.92}

% [CONFERIR - Victor] Esta captura e de uma execucao anterior: ela mostra
% "ID: 12 / Joarez", enquanto a fila e o historico (Figura 12) registram
% "13 - Juarez". A legenda abaixo NAO cita identificador nem grafia, para
% nao contradizer as outras figuras. Se refizer o print, pode voltar a
% citar os nomes aqui.
\tela{03-cliente-a-cadastro.png}{Cliente A. A captura mostra a sessão inteira: \texttt{Conectado ao servidor.}, a resposta \texttt{LOGIN\_OK}, o menu de quatro opções e dois cadastros feitos em sequência, cada um confirmado pelo servidor.}{0.72}

\telaDupla%
  {04-cliente-b-broadcast.png}{Cliente B recebe os cadastros feitos em A}
  {05-ver-fila.png}{Cliente A recebe os cadastros feitos em B e pede a fila}
  {Notificação em tempo real, vista dos dois lados. Nenhum dos dois clientes pediu essas linhas: elas chegaram sozinhas, assim que o outro cadastrou. À direita, a opção \texttt{2} logo depois, com os quatro usuários entre o cabeçalho e o rodapé.}

\telaDupla%
  {06-heartbeat-novidade.png}{\textit{Heartbeat} com novidade}
  {07-heartbeat-alive.png}{\textit{Heartbeat} sem novidade}
  {Comportamento do \textit{heartbeat} no cliente A, conforme a Observação do enunciado. À esquerda, devolve apenas \texttt{21 - Jonas Baleia} e \texttt{11 - Pedro Casagrande}, cadastrados pelo cliente B. À direita, a mesma opção repetida em seguida devolve \texttt{ALIVE}, porque nada mudou desde a verificação anterior.}

\tela{08-caso-de-erro.png}{Validação no cliente. O identificador \texttt{-7} é recusado antes mesmo de virar mensagem de rede, e o operador volta ao menu com a conexão intacta. O servidor faz a mesma checagem por conta própria, como mostram os casos 14 e 15 da Tabela~\ref{tab:casos}.}{0.62}

\tela{09-persistencia.png}{Persistência em arquivo. O \texttt{fila.txt} guarda o retrato atual da fila, no mesmo formato da resposta de \texttt{LIST}; o \texttt{historico.txt} guarda o registro permanente de cada inserção, com data e hora.\label{fig:persistencia}}{0.92}
...
as orientações e as exigências do enunciado: cliente e servidor são iniciados sem
parâmetros, o servidor atende vários clientes ao mesmo tempo, a fila é atualizada
em tempo real e os dados ficam gravados em arquivo. Todos os testes previstos
foram executados com sucesso, inclusive o de carga com 10.000 clientes.

A parte mais difícil do desenvolvimento foi delimitar as mensagens. Os códigos
vistos em aula trocam uma única mensagem por conexão, então nenhum deles precisa
resolver esse problema. Aqui a conexão fica aberta e carrega dezenas de
mensagens, e o TCP entrega os bytes sem separar uma da outra. Foi preciso criar a
convenção de uma linha por mensagem e uma leitura que vai juntando os bytes
recebidos até a linha ficar completa.

Escolher a técnica de múltiplos clientes também deu trabalho. O enunciado permite
três caminhos: multiplexação, \texttt{fork()} e threads. Deixei a multiplexação
de lado por ser a técnica com que tive menos contato: implementá-la e depois
saber explicá-la com segurança levaria mais tempo do que eu tinha. O
\texttt{fork()} parecia o caminho natural, mas processos têm memória separada, e
compartilhar a fila entre eles exigiria comunicação entre processos. As threads
compartilham a memória sozinhas, e foi por isso que as escolhi. Em troca, foi
preciso proteger a fila com um \textit{mutex}.

O teste de 10.000 clientes não funcionou na primeira tentativa. O programa parava
por falta de memória, e demorei para entender o motivo: cada thread reserva 8~MB
de pilha por padrão, o que daria 80~GB só de espaço reservado. Reduzir a pilha
para 256~KB resolveu, e o teste passou sem nenhuma falha.

Ao longo do caminho surgiram várias ideias que eu gostaria de ter colocado em
prática, mas o prazo não deixou. A que mais me deixou curioso foi a
multiplexação: seria muito interessante construir o mesmo sistema das duas
maneiras e comparar quanto cada uma custa por cliente. Também pensei em ler o
endereço do servidor de um arquivo de configuração, o que evitaria recompilar a
cada troca de máquina, e em substituir o vetor fixo por uma estrutura que cresce
sozinha, tirando o limite de 20.000 registros. Fica a vontade de voltar ao código
depois da entrega e experimentar cada uma delas.

% =============================================================================
% REFERENCIAS
% =============================================================================
% O ambiente thebibliography ja cria o proprio titulo de secao, usando o
% \refname definido no preambulo. Nao escrever o titulo a mao aqui - fazer
% isso fazia aparecer dois titulos seguidos no PDF.
...
\bibitem{aula}
BARBOSA FILHO, Rubens. \textit{Códigos de exemplo da disciplina de Redes de
...
% ==================================================================
% Nota sobre a entrada acima: em ABNT, os sufixos Filho, Neto, Junior
% e Sobrinho acompanham o ultimo sobrenome. Por isso "Rubens Barbosa
% Filho" entra como BARBOSA FILHO, Rubens - e nao BARBOSA, Rubens F.
%
% Se voce consultou algum livro (Tanenbaum, Kurose, Stevens) ou algum
% slide da disciplina durante o desenvolvimento, acrescente aqui. Se
% NAO consultou, nao invente: a lista acima ja cobre as fontes que o
% enunciado pede e as que o codigo realmente usou.
% ==================================================================

\end{thebibliography}

% =============================================================================
% CAPTURAS USADAS E CAPTURAS DE RESERVA
%
% O documento usa 10 capturas, varias em pares lado a lado:
%
%   Item 7 (funcionamento)
%     01-servidor-iniciado.png
%     03-cliente-a-cadastro.png
%     04-cliente-b-broadcast.png  +  05-ver-fila.png      (par)
%     06-heartbeat-novidade.png   +  07-heartbeat-alive.png (par)
%     08-caso-de-erro.png
%     09-persistencia.png
%
%   Item 6 (carga)
%     carga-inicio-100.png    +  carga-fim-100.png        (par)
%     carga-inicio-1000.png   +  carga-fim-1000.png       (par)
%     carga-fim-10000.png     +  carga-cliente-fim-10000.png (par)
%     carga-cliente-1000.png
%
% NAO usadas, ficaram de reserva na pasta figuras/:
%     02-cliente-a-login-menu.png   (o conteudo dela ja aparece na 03)
%     carga-cliente-100.png         (equivalente a carga-cliente-1000)
%     carga-cliente-10000.png       (so o inicio; o fecho esta na -fim-)
%
% Se quiser trocar alguma, e so mudar o nome do arquivo na chamada
% \tela{} ou \telaDupla{} da secao correspondente.
%
% -----------------------------------------------------------------------
